 ...
 */
function get_lazy_loaded_custom_logo() {
    $custom_logo_id = get_theme_mod( 'custom_logo' ); 
    if ( ! $custom_logo_id ) {
        return ''; 
    }
    return get_image_attrachment( $custom_logo_id, 'image' ); 
}
...
function create_taxonomy($key, $config = array())
{
	...
	$labels = array(
		...
		'menu_name' => $config['name'],
		'all_items' => 'Tất cả ' . $config['name'],
		'edit_item' => 'Chỉnh sửa ' . $config['name'],
		'view_item' => 'Xem ' . $config['name'],
		'update_item' => 'Cập nhật ' . $config['name'],
		'add_new_item' => 'Thêm ' . $config['name'] . ' mới',
		'new_item_name' => 'Tên ' . $config['name'] . ' mới',
