                        ...
                        <?php if($tabs): ?>
                            <?php foreach($tabs as $index => $item): ?>
                                <li class="events-header-tab-item <?= $index == 0 ? 'active' : '' ?>">
                                    <a href="<?= $item['link']['url'] ?>" target="<?= $item['link']['target'] ?>"><?= $item['text'] ?></a>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </ul>
                    ...
                    <div class="block-news-first">
                        <div class="news-item-big zoom-img-parent">
                            <div class="block-thumb"> 
                                <div class="thumb img-zoom"><img class="lozad" data-src="<?= get_the_post_thumbnail_url($first_news->ID, 'full') ?>" alt="<?= get_the_title($first_news->ID) ?>"/>
                                </div>
                            </div>
                            <div class="news-info"> 
                                ...
                                </div><a class="btn btn-white btn-seemore" href="<?= get_permalink($first_news->ID) ?>">Xem chi tiết</a>
                            </div>
                        </div>
                    </div>
                    <div class="side-news"> 
                        ...
                        <div class="news-item zoom-img-parent h-full">
                            <div class="block-thumb"> 
                                <div class="thumb img-zoom"><img class="lozad" data-src="<?= get_the_post_thumbnail_url($item->ID, 'full') ?>" alt="<?= get_the_title($item->ID) ?>"/>
                                </div>
                            </div>
                            <div class="news-info"> 
                                <h3 class="info-name"><?= get_the_title($item->ID) ?></h3><a class="btn btn-white btn-seemore" href="<?= get_permalink($item->ID) ?>">Xem chi tiết</a>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
